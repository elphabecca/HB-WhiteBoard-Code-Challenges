...
class Node(object):
    ...
    def cousins(self):
        ...
        def find_depth(self, depth=0):
            while self.parent:
                depth+=1
                return find_depth(self.parent, depth)
            return depth, self

        depth, root = find_depth(self)

        node_dict = {}

        def create_node_dict(node, depth=0):
            node_dict[node] = depth
            if node.children:
                depth+=1
                for node in node.children:
                    create_node_dict(node, depth)

        create_node_dict(root)

        cousins = [k for k, v in node_dict.items() if v == depth]
        cousins.remove(self)
